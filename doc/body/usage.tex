...
\begin{function}[added = 2022-09-12]{page/show-foot}
  \begin{ccnusyntax}[emph={[1]show-foot}]
    show-foot = (*\TTF*)
  \end{ccnusyntax}
  是否显示页脚。
\end{function}
...
\begin{itemize}
  \item \emph{v0.2.6 修复：}修复了 \env{solution} 环境与 |align*| 等数学环境结合时的尾随空行问题。若您在旧版本中遇到数学环境后出现异常空白，请更新到最新版本。
  \item 在 \env{solution} 环境中使用 \env{align*}、\env{gather*} 等 |amsmath| 数学环境时，现已能正确处理环境结尾，不会产生多余空行。
\end{itemize}
...
\begin{function}[added = 2022-07-03, updated = 2025-01-07]{draft}
  \begin{ccnusyntax}[emph={[1]draft}]
    draft = (*\marg{键值列表}*)
    draft/(*\meta{key}*) = (*\meta{value}*)
  \end{ccnusyntax}
  该选项包含许多子项目。用于设置 \tn{draftpaper} 命令。\emph{v0.2.6 起支持多层级键值设置}，可使用 |draft = { ... }| 或 |draft/key = value| 两种等价写法。例如：
  \begin{latexcode}
% 方式 1：使用嵌套键值列表
\examsetup{
  draft = {
...
\examsetup{
  ...
  \end{latexcode}
  下面所述的键值，如果不额外说明，则表示可以通过 \tn{examsetup} 进行统一处理，也可以用于 \tn{draftpaper} 命令的 \meta{键值列表} 针对某一 \tn{draftpaper} 命令调整。具体内容见下。
\end{function}
...
\begin{function}[added = 2024-02-11, updated = 2025-01-07]{calculations/hsep,calculations/vsep}
  ...
  \emph{注：v0.2.6 起，内部实现优化了双列布局时的列宽计算（从 0.3 倍文本宽度调整为 0.45 倍），改善了排版效果。用户无需修改已有代码。}
\end{function}
